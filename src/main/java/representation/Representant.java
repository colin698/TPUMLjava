...
public class Representant {
    ...
    private float[] chiffreAffaire = new float[12];
    ...
    public float salaireMensuel(int mois, float pourcentage) {
        if (pourcentage < 0) {
            throw new IllegalArgumentException("LE POURCENTAGE DOIT ETRE POSITIF OU NUL");
        }

        float salaire;

        salaire = this.salaireFixe + this.chiffreAffaire[mois] * pourcentage + this.secteur.getIndemniteRepas();
        return salaire;
    }
